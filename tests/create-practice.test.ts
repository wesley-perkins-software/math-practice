...
export const tests = [
  ...
  test('selected multiplication facts validate, normalize, and serialize live', () => {
    ...
    assert.equal(derive({ ...base, skillOptions: { facts: [6] } }).query, 'v=2&skill=multiplication-facts&facts=6&mode=untimed&problems=20');
    const multiple = derive({ ...base, skillOptions: { facts: [8, 6, 7] } });
    assert.equal(multiple.query, 'v=2&skill=multiplication-facts&facts=6,7,8&mode=untimed&problems=20');
  ...
  test('selected division values validate, normalize, and serialize live', () => {
    ...
    assert.equal(derive({ ...base, skillOptions: { divisors: [8, 6, 7] } }).query, 'v=2&skill=division-facts&divisors=6,7,8&mode=untimed&problems=20');
  ...
  test('session controls produce only supported untimed, endless, timed, and finite definitions', () => {
    ...
    assert.equal(derive(base).query, 'v=2&skill=addition-1-digit&mode=untimed&problems=20');
    ...
    assert.equal(derive({ ...base, mode: 'timed', durationSeconds: 120 }).query, 'v=2&skill=addition-1-digit&mode=timed&duration=120&problems=20');
  ...
  test('there is no applied snapshot: every edit updates the one copy/start target', () => {
    ...
    assert.equal(first.relativeUrl, '/practice/?v=2&skill=multiplication-facts&facts=6,7&mode=untimed&problems=20');
    assert.equal(second.relativeUrl, '/practice/?v=2&skill=multiplication-facts&facts=8,9&mode=untimed&problems=30');
