...
export const tests=[
 ...
 test('global nav and operation category hubs remain unchanged by discoverability work',()=>{const header=source('src/components/SiteHeader.astro');assert.equal(header.includes('/create'),false);for(const p of ['src/pages/addition/index.astro','src/pages/subtraction/index.astro','src/pages/multiplication/index.astro','src/pages/division/index.astro'])assert.equal(source(p).includes('/create'),false,p)})
];
