...
- Phase 2 pilot (`[table].astro`, tables 2/7/9 only): **complete** — see [Phase 2: Pilot on Times Tables 2, 7, 9](#phase-2-pilot-on-times-tables-2-7-9-complete) below
- Live generated-page integration for the remaining 9 multiplication tables, `.../index.astro`, and `[divisor].astro`: **not started** — blocked on the pilot's open questions, not on missing architecture
- Divide-by fact bank (Phase 1B): **not started**
- Generated-page rollout (Phase 3+): **not started**

This document, the Phase 1A fact bank, and the Phase 2 pilot together are architecture-plus-authored-data-plus-one-validated-implementation. The fact bank is real, authored, validated data — not a placeholder or a design sketch — and as of Phase 2 it is read by 3 of the 12 live times-table pages (2, 7, 9); the other 9 still use their pre-existing local content.
...
- **Scope boundary:** This document plus Phase 1A was architecture and authored data only, not page-template implementation. Phase 2 (below) is the first page-template implementation, deliberately scoped to a 3-table pilot (`[table].astro`, tables 2, 7, 9) rather than a full rollout. `src/pages/multiplication/times-tables/index.astro`, `src/pages/division/divide-by/[divisor].astro`, `PracticeLayout.astro`, and `presets.ts` remain untouched, as do the other 9 multiplication tables within `[table].astro` itself — all of these still use their pre-existing local `TABLE_STRATEGIES`/`STRATEGY`/`GRADE_BADGE`/inline FAQ content. See [Phased rollout plan](#phased-rollout-plan) for how the remaining phases get built.
...
| 12 | Derived-fact method | Split into 10× and 2× | Isolating the addition step as the likely error source; naming successful strategy-combination as a milestone | Worked example of the split strategy | No — inherits family default (revised after review; see below) |
...
- The grade-override rationale for 11 and 12 (introduced in this fact bank, not present in the old `GRADE_BADGE` map) is a new, hedged claim, not one lifted from existing project documentation — reviewed and revised below rather than left as originally drafted.

**Post-review revisions (approved before merge):**
- **Table 11 `gradeOverride`** reworded from *"Often reinforced once the 1–10 tables are secure, since recognizing the two-digit exceptions draws on place-value understanding typically solidified slightly later"* to *"Often reinforced after the 1–10 tables are secure, since the larger 11s facts require students to move beyond the familiar repeated-digit pattern and apply place-value understanding"* — clearer, more mathematically grounded, and drops the vague "two-digit exceptions" phrasing.
- **Table 12 `gradeOverride` removed entirely.** On review, there was insufficient justification for giving table 12 its own curriculum-timing claim beyond the family-level default (`TIMES_TABLE_GRADE_DEFAULT`) — the original override conflated a genuine instructional observation (combining two strategies) with an unsupported timing claim. Table 12 now inherits the family default. The instructional idea was preserved, not discarded: `parentTeacherNote` now also names successfully combining the ×10 and ×2 strategies as a milestone worth calling out, which is where that observation belongs (diagnostic/instructional guidance, not a curriculum-timing claim).
...
## Phase 2: Pilot on Times Tables 2, 7, 9 — complete

**Status:** Complete. This phase wired the full canonical content pattern (Quick Answer, expanded intro, Strategy, Parent & Teacher Guidance, FAQ differentiator) into the live `[table].astro` route, scoped deliberately to 3 of the 12 tables — not a template redesign and not a rollout to the remaining 9.

**Files changed:** `src/pages/multiplication/times-tables/[table].astro` only. No other page, layout, or preset was modified.

**Mechanism:** `const fact = PILOT_TABLES.has(tableNumber) ? getTimesTableFact(tableNumber) : undefined`, where `PILOT_TABLES = new Set([2, 7, 9])`. Every section that reads from the fact bank is conditioned on `fact` being defined and falls back to the pre-existing `TABLE_STRATEGIES`/`GRADE_BADGE`/`introLead`/`introSupport` content otherwise, so the other 9 tables' rendered output is unaffected. Verified: diffing built HTML for tables 1, 3, 4, 5, 6, 8, 10, 11, 12, and `times-tables/index.astro` against a pre-change build shows no difference beyond incidental whitespace from Astro's conditional-block compilation.

**Section-by-section implementation:**
- **Quick Answer:** `"The {n} times table means multiplying {n} by each whole number from 1 to 12. {quickAnswerFact}"` — matches this document's specified skeleton-plus-fact pattern exactly.
- **Expanded intro:** hand-composed per pilot table (not template-generated), weaving each table's `introClause` into a ~44-48 word paragraph. The legacy `introLead`/`introSupport` pair was dropped for pilot tables rather than stacked alongside the new content, to avoid three repetitive prose blocks in a row and to stay inside the standard's 40-70 word target.
- **Strategy:** `strategyTitle`/`strategyExplanation` feed the exact same `strategy.name`/`strategy.tip` template variables the legacy `TABLE_STRATEGIES` object used — no markup changes needed for this section.
- **Parent & Teacher Guidance:** new section added between Strategy and FAQ, present only for pilot tables, built from a one-sentence skeleton plus `parentTeacherNote` verbatim.
- **FAQ:** `faqDifferentiator` appended as a 5th item for pilot tables only; the existing `FAQPage` JSON-LD generation (which maps over the shared `faqItems` array) required no separate change.
- **Grade badge / `educationalLevel`:** deliberately left on the legacy `GRADE_BADGE` path for all 12 tables, pilot included — see the grade-metadata finding below.

**Validation performed:**
- `npm run validate:times-tables` — passes, 12/12 entries valid.
- `npm run build` — succeeded, 68 pages, no errors, same page count as before.
- Built and diffed `dist/` output before/after for all 9 non-pilot tables and `index.astro` — unchanged except incidental whitespace.
- Inspected `dist/` HTML for tables 2, 7, 9 — JSON-LD (`LearningResource`, `FAQPage`) valid, FAQ schema count matches visible FAQ count (5), canonical/title/grade-badge/`educationalLevel` unchanged, heading hierarchy clean.
- Visually inspected all three pilot pages and one non-pilot page (5) at 1280×900 (desktop) and 390×844 (mobile) via a local dev server, specifically to evaluate fold position (see findings below).

**Findings:**

1. **Field coverage is sufficient.** All six fields used by this section order (`quickAnswerFact`, `introClause`, `strategyTitle`, `strategyExplanation`, `parentTeacherNote`, `faqDifferentiator`) mapped onto a real template section with no gaps. This is the strongest validation yet that the Phase 1A field design was correct — it held up under actual template integration, not just in isolation.
2. **Fold position is a real, measurable cost.** Adding Quick Answer + expanded intro visibly pushes the practice widget down the page on every pilot table. At a 900px desktop viewport, the grade badge that sits just below the widget on a non-pilot page (table 5) is at the very edge of the fold; on pilot pages it's pushed below the fold entirely. On a 390×844 mobile viewport, only the widget's tab switcher remains visible within the fold on pilot pages, versus the widget's full picker plus the start of "Related Practice" on the non-pilot page. This is the standard's biggest open risk for a 12-table rollout and should be explicitly decided (accept the tradeoff, or tighten the intro further), not silently inherited from this pilot.
3. **Grade display metadata likely belongs outside `GeneratedPracticeEntry`.** The interface's grade fields (`gradeOverride`, `TIMES_TABLE_GRADE_DEFAULT`) are prose, correctly shaped for narrative sections — but this page also needs a short, structured value (`"Grade 2 – Grade 3"`) for a UI pill and for `educationalLevel` in JSON-LD, and no such field exists or was added here. Recommendation for whoever scopes the next phase: treat grade *display* metadata as family-level template configuration, separate from the educational fact bank, rather than adding a compact-label field to `GeneratedPracticeEntry` merely because one page happens to show a badge. Not decided in this pilot — left on the legacy path for all 12 tables so no inconsistency was introduced.
4. **One field-length imbalance found:** table 7's `strategyExplanation` (~75 words) is noticeably longer than the neighboring static "Use Commutativity" card in the 2-column strategy grid, visibly unbalancing that specific table's layout (tables 2 and 9 are comparable in length to the static card). Not a defect in the field itself — table 7's honest "no shortcut" content genuinely needs more words than a one-line trick — but worth a content-length guideline before a 12-table rollout produces more of these mismatches.
5. **Pre-existing bug surfaced, not introduced, by this pilot:** the static "Use Commutativity" card's copy is hardcoded as `If you know 7×{tableNumber}, you already know {tableNumber}×7`, using a literal `7` rather than a second distinct reference number. For table 7 specifically this renders as "If you know 7×7, you already know 7×7" — a tautology. This bug exists on all 12 tables today (independent of this pilot) but is unusually visible on table 7. Flagged here for a future fix; not fixed in this pilot since it's outside the fact-bank migration's scope and would touch all 12 tables' output.
6. **Differentiation confirmed.** Tables 2, 7, 9 read as genuinely distinct from each other and from the remaining legacy tables — none of the Quick Answer, Strategy, or FAQ differentiator content would read as true if swapped between pages.

**Go/no-go recommendation:** **Not yet ready for a full 9-table rollout as-is.** The content-field mapping needs no changes — that part of the standard is validated. Two things should be resolved first: (a) an explicit decision on the fold-position tradeoff (finding 2), and (b) a decision on where grade display metadata should live (finding 3), so all 12 tables end up on one consistent mechanism instead of 3 pages having grade metadata separated from the fact bank by pilot accident rather than by design. Neither finding invalidates the architecture; both are exactly the kind of thing a pilot is supposed to surface before a wider rollout.
...
- **Phase 2:** Pilot the refactor on a small number of `[table].astro` instances — wire the new data modules and full section order into the existing dynamic-route template, verify build output and JSON-LD validity. **Complete**, scoped to tables 2, 7, and 9 — see [Phase 2: Pilot on Times Tables 2, 7, 9](#phase-2-pilot-on-times-tables-2-7-9-complete). The flagged `times-tables/index.astro` "hardest table" content-integrity issue was **not** resolved in this phase (that file was not touched at all, per this phase's explicit scope) — still open, see the open questions below.
- **Phase 3:** Roll out to the remaining 9 multiplication tables (and, separately, `[divisor].astro`/Divide By) once the pilot's two open findings — fold position and grade-metadata placement — are explicitly resolved, following the same "ship as piloted, log deviations" discipline used in the leaf-page rollout. **Not started.**
...
2. ~~**Who authors the data-module entries, and how are they fact-checked?**~~ — **Answered for Phase 1A** (this task): every numeric example was recalculated, digit-pattern claims were verified algebraically, and entries were checked against the standard's review checklist plus programmatic validation.
3. **Divide-by index page ownership** — should `division/divide-by/index.astro` be built as part of the eventual template rollout (Phase 3/4) or tracked as a fully separate PR, given it's already an independent roadmap item in `SEO_AEO_GEO_AUDIT.md`?
...
6. ~~**Should the 11/12 grade overrides be validated further?**~~ — **Resolved.** Table 11's override was reworded for clarity and mathematical grounding; table 12's override was removed entirely (inherits the family default) after review found insufficient justification for a table-12-specific curriculum-timing claim. See "Post-review revisions" above.
7. **How should the flagged `times-tables/index.astro` "hardest table" FAQ content be handled?** Still open — the Phase 2 pilot deliberately did not touch `index.astro` (out of its 3-table scope), so this pre-existing, unsupported comparative claim remains live on production content today. Should be resolved no later than the Phase 3 rollout, and arguably sooner.
8. **New, from Phase 2 — should grade display metadata (the compact badge/`educationalLevel` value) become a family-level template concern separate from `GeneratedPracticeEntry`, and if so, what should that mechanism look like?** The pilot found the existing prose-shaped grade fields (`gradeOverride`, `TIMES_TABLE_GRADE_DEFAULT`) are the wrong shape for a compact pill/JSON-LD string, but deliberately did not add a new field to answer this — see [Phase 2](#phase-2-pilot-on-times-tables-2-7-9-complete) finding 3. Must be resolved before Phase 3 so all 12 tables share one mechanism.
9. **New, from Phase 2 — is the fold-position cost of Quick Answer + expanded intro acceptable across all 12 tables,** or does the intro need to be tightened further (shorter Quick Answer, shorter intro, or both) before Phase 3? See [Phase 2](#phase-2-pilot-on-times-tables-2-7-9-complete) finding 2 for the measured desktop/mobile impact.
10. **New, from Phase 2 — should the pre-existing "Use Commutativity" card bug** (hardcoded literal `7` producing a tautology for table 7: "If you know 7×7, you already know 7×7") **be fixed as part of Phase 3, or as a fully separate, smaller fix** given it affects all 12 tables' static copy independent of the fact-bank migration?
