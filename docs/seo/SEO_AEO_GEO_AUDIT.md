...
- **Created:** 2026-08-02
- **Last updated:** 2026-09-12 — see "Current state as of September 2026" note directly below. Several items this document previously described as pilot-only or not-started are, as of a source-level discoverability/SEO audit on 2026-09-12, confirmed fully shipped. Everything below the note is otherwise preserved as historical record per this doc's own maintenance rule (deferred recommendations aren't deleted for being old); only status markers and the specific stale claims are corrected in place.

### Current state as of September 2026

A repository-aware discoverability/SEO/AEO/GEO audit (2026-09-12) found this document materially out of date relative to `development`. Corrected facts:

- **The division divisor grid (Section 5's item #3, previously "Deferred (approval gate)") is live.** `src/pages/division/divide-by/index.astro` renders a full 1–12 divisor grid identical in pattern to `multiplication/times-tables/index.astro`. This closes the deepest-page-on-the-site finding from the original deep-dive audit (Section "Deep-dive audit: additional findings").
- **The Divide By fact bank (Phase 1B, previously "not started") is authored and wired in.** `src/data/generated-practice/divideBy.ts` exists alongside `timesTables.ts` and is imported by `src/pages/division/divide-by/[divisor].astro` via `getDivideByFact()`.
- **The Canonical Generated-Page Standard rollout (Phase 2/3, previously "3-table pilot only... not yet ready to roll out to the remaining 9 tables") is complete for all 12 multiplication tables**, not just tables 2, 7, and 9. `src/pages/multiplication/times-tables/[table].astro` calls `getTimesTableFact(tableNumber)` unconditionally for every table 1–12; the `fact`-gated conditionals in that file are now vestigial (every table has a fact-bank entry) but functionally harmless — flagged as a future code-clarity cleanup, not a content or SEO issue.
- New product surfaces not covered anywhere in this document at all — Multiplication Test (`/multiplication/test/`), Multiplication Chart (`/multiplication-chart/`), and the Daily Review hub + 6 grade routes (`/daily-review/...`) — were audited separately on 2026-09-12 and found to already meet this document's own standards (title/meta/canonical hygiene, FAQPage + LearningResource + BreadcrumbList schema, answer-first AEO copy, differentiated anchor text). Two small link-graph gaps were found and fixed in that same pass: Daily Review was missing from the shared `HubLayout`/`PracticeLayout` footer (present only in the homepage's own footer), and Arithmetic Speed Drill had no inbound link from either the Multiplication or Division hub. Both fixed directly rather than re-logged here as roadmap items, since they were same-day, single-line fixes.
- **Do not treat any roadmap item below marked "not started" or "pilot only" as still accurate without checking current source first** — this document lagged actual implementation by roughly six weeks before this correction pass.
...
- [x] Phase 1B — author the divide-by fact bank (`src/data/generated-practice/divideBy.ts`) — **confirmed complete as of 2026-09-12.** The module exists and is imported by `src/pages/division/divide-by/[divisor].astro` via `getDivideByFact()`. (Not implemented in the PR this line originally tracked; found already done in a later source-level audit — see "Current state as of September 2026" note at the top of this document.)
- [x] Phase 2 — pilot the full canonical content pattern on a small number of `[table].astro` instances — **completed, scoped to tables 2, 7, and 9 only** at the time this line was written. See "Phase 2 pilot" write-up below for implementation, validation, and evaluation.
- [x] Migrate `.../index.astro` / `[divisor].astro` and roll the standard out to the remaining 9 multiplication tables (plus Divide By) — **confirmed complete as of 2026-09-12.** `src/pages/multiplication/times-tables/[table].astro` calls `getTimesTableFact()` unconditionally for all 12 tables; `[divisor].astro` calls `getDivideByFact()` for all 12 divisors. (Also found already done in the same later audit, not tracked step-by-step here — see the top-of-document note.)
...
- [x] **Give division a divisor grid, matching multiplication's times-table index** — *Quick Win.* **Confirmed complete as of 2026-09-12.** `src/pages/division/divide-by/index.astro` now renders a full 1–12 divisor grid, linked from `division/index.astro`, mirroring `multiplication/times-tables/index.astro`. This closes the `/division/divide-by/8` crawl-depth outlier described below.
...
- [x] **Fix the `division/divide-by/8` crawl-depth outlier** — tracked above as the division divisor grid item, now confirmed complete (2026-09-12); the divisor grid brings every `divide-by/[divisor]` page back to crawl depth 3.
...
*(Rewriting the homepage FAQ was previously listed here; explicit approval was given and it was completed — see [Section 4b](#4b-homepage-faq-single-source-of-truth-math-drills-correction--about-page-organization-cleanup-✅-implemented). The division divisor grid was also previously listed here; it shipped between the last full pass of this document and 2026-09-12 — see the top-of-document "Current state" note — and is removed from this gate accordingly.)*
...
| 13 | Canonical Generated-Page Standard — architecture, fact banks, and full 12-table/12-divisor rollout | Medium | ✅ Confirmed fully complete as of 2026-09-12 — architecture, both fact banks (`timesTables.ts` and `divideBy.ts`), and the page-template rollout to all 12 times-table leaves and all 12 divide-by leaves. | Closes item 12's generated-page extension entirely. See `docs/seo/CANONICAL_GENERATED_PAGE_STANDARD.md` for the architecture record; see the "Current state as of September 2026" note at the top of this document for how the remaining phases were found already shipped. |
...
| 2026-09-12 | Pending | New-feature discoverability audit (Multiplication Test, Multiplication Chart, Daily Review hub + grades, Create, Divide By hub/leaves) found this document stale — Phase 1B, the full 12-table/12-divisor rollout, and the division divisor grid had all shipped since the last update without this doc being corrected. Corrected the status markers/checkboxes throughout and added a "Current state as of September 2026" note at the top. Also shipped two small fixes from the same audit: added `/daily-review/` to the shared `HubLayout.astro`/`PracticeLayout.astro` footer (previously present only in the homepage's own inline footer); added an Arithmetic Speed Drill link to both `/multiplication/` and `/division/` hubs' Related Practice sections (previously reachable only via `/multiplication/test/` and `/division/remainders/`) | Doc-correction pass plus two single-line, additive link-graph fixes. No titles, canonicals, schema types, or page structure changed. |
