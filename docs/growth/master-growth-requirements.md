...
- **Tier:** A · **Status:** Complete · **Mechanism:** Acquisition + Distribution + Retention
- **Completion note (2026-09-11):** shipped as `/daily-review/` — one canonical, evergreen, indexed page (no dated archive, no per-grade routes). Six grade selections (Kindergarten, Grade 1–5), each a deterministic 10-question arithmetic set built from local calendar date + grade via a dedicated `generateDailyReviewProblems` composition function (`src/engine/dailyReview.ts`) — explicit per-grade operation quotas (not generic `operation:'mixed'` random selection), one seeded RNG, one isolated `GenerationHistory`, one deterministic shuffle. That single ordered `Problem[]` is the sole source of truth for interactive practice and same-day replay. `PracticeWidget` gained one small additive `problems?: Problem[]` prop (existing callers unaffected) plus one source-audited runtime fix that was a prerequisite for Grade 5's exact/remainder division mix: remainder-input rendering now reads `problem.remainder` instead of session-wide `config.withRemainder` (verified behavior-preserving for every existing page). PB/streak/progress-link UI is suppressed via the existing `sessionPresentation="shared"` path; Daily Review intentionally stays out of `/progress/`/`ALL_PRESETS` in V1, with per-grade completion state stored independently (completing one grade never marks another complete). No secondary shuffle/new-set action, no projector-mode toggle (the existing prototype surface is already classroom-display-capable), untimed only, no accounts/backend. **Post-launch QA revision:** the first pass introduced problem shapes (3-digit addition/subtraction, 2-digit × 2-digit and 1-digit × 2-digit multiplication) that don't exist anywhere else as a shipped interactive practice format, and a widened `WrittenProblemInput` answer cap to support them — both were removed; every grade now reuses only arithmetic shapes already established elsewhere on the site (facts-mode multiplication/division, the existing `selectedFacts` harder-pool mechanism, and the already-shipped Division with Remainders shape), the answer cap was reverted to 3 digits, and `DAILY_REVIEW_MIX_VERSION` was bumped so the revised sets are treated as a new deterministic composition. That same QA pass also replaced an initial six-large-card grade picker with a compact `K 1 2 3 4 5` pill selector so the practice widget, not grade configuration, is the dominant above-the-fold element, and **removed print entirely from V1** (no Print Review/Print Answer Key) — printing the interactive widget was not a polished experience, and the site's dedicated Worksheets product remains the printable-practice option; Daily Review V1 is intentionally interactive-only. Discoverability: a homepage callout, a For Teachers classroom-workflow step and link, and a grade-prefilled `?grade=` CTA on all five existing grade pages (client-side-only preselection; canonical stays query-string-agnostic). Tests in `tests/daily-review.test.ts` cover exact quotas, operand ranges, no-negative-subtraction, exact/remainder division correctness, an explicit assertion that no grade emits a problem shape outside established site formats, determinism across date/grade/mix-version, and independent per-grade completion storage; `tests/routes-components.test.ts` and `scripts/test-build-contract.mjs` extended for route/discoverability/canonical/sitemap coverage. Full suite passing, clean `npm run build`, `npm run test:build-contract` passing.
- **Follow-up (2026-09-11) — hub + grade-specific routing:** visual QA on the single-page `/daily-review/` shape found that the in-page six-grade selector competed with the practice widget for above-the-fold space. Grade selection was moved out of widget state and into navigation: `/daily-review/` is now a lightweight hub (breadcrumb, intro, six grade cards, "how it works"/classroom-use copy, FAQ — no practice widget), and six stable static routes — `/daily-review/kindergarten/`, `/daily-review/1st-grade/`, `/daily-review/2nd-grade/`, `/daily-review/3rd-grade/`, `/daily-review/4th-grade/`, `/daily-review/5th-grade/` — each render `DailyReviewWidget` immediately after a one-line, grade-specific intro, with compact "Other Daily Reviews" navigation and related links placed below the practice experience, never above it. The six pages share one `src/pages/daily-review/[grade].astro` template with `getStaticPaths()` mapping each natural-language slug to the existing stable engine grade ID (`k`/`g1`…`g5`); `DailyReviewWidget` now takes a `gradeId` prop instead of owning grade state, and no longer parses `?grade=`, renders a grade selector, or writes a `lastSelection` preference. Deterministic generation, per-grade quotas, seeded RNG, `GenerationHistory`, the prebuilt `Problem[]` path, same-day replay, per-grade completion storage, and `sessionPresentation="shared"` PB/streak suppression are all unchanged. `daily_review_grade_select` was removed (selection is now navigation, not an in-widget action); `daily_review_view`/`start`/`complete` are unchanged. `DailyReviewPrefs` drops the now-unread `lastSelection` field; a record persisted before this change simply carries it as an inert extra key — no migration or version bump needed. Each grade page self-canonicalizes and is indexable; all seven Daily Review URLs (hub + six grades) are in the sitemap with no query-string variants. No compatibility redirect was added for old `/daily-review/?grade=gN` links — the feature had not meaningfully launched, so no such redirect infrastructure was worth the added surface area. Interactive-only V1 and no-print scope are unchanged.
...
- **Implementation requirements (own product/architecture validation still applies before implementation — this is about scope, not about waiting on the Chart):**
  - [x] Confirm the deterministic mixed-set/config approach (grade-level selection, mixed operations) against the existing `PracticeConfig` shape.
  - [x] Decide projector-mode and printable-companion scope up front (per the owner's preference for a polished launch, not a deliberately weak MVP), even though the internal build order below stages the work.
...
6. **GROWTH-009** *(Tier A — execute)* — Daily/Mixed Review warm-up tool (queued after the Chart for bandwidth, not evidence) — **Complete**
...
4. GROWTH-002 *(Tier A)*
5. GROWTH-003 *(Tier A)*
6. GROWTH-009 *(Tier A — queued after GROWTH-006 for bandwidth, not evidence)* — **Complete**
...
- **Next 30 days:** GROWTH-004 (tiny structural fix) shipped; GROWTH-005A (trailing-slash hygiene) also shipped in this pass, ahead of its originally sequenced position, since it was low-risk and ready; a small follow-on IA cleanup also shipped in this pass — family-level "Times Tables" links (multiplication hub's primary mode card, two homepage grade-band cards, and the Multiplication Facts related-links/CTA) were repointed from `/multiplication/times-tables/1/` to the `/multiplication/times-tables/` hub, matching the divide-by pattern GROWTH-004 established; genuinely leaf-specific links (e.g. the For Teachers entry-flow CTA, hub tips prose citing specific tables) were left unchanged; GROWTH-001 (baseline) remains pending — attempted on mobile, deferred until desktop access is available; substantial, and ideally complete, progress on **GROWTH-006 (Multiplication Chart)** — this is a near-term build, not a later-quarter one. GROWTH-002 (Divide-By 6–12 maturity fix) can run in parallel given its small, independent scope.
