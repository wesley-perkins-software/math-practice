...
- **Completion note (2026-09-11):** shipped as `/daily-review/` — one canonical, evergreen, indexed page (no dated archive, no per-grade routes). Six grade selections (Kindergarten, Grade 1–5), each a deterministic 10-question set built from local calendar date + grade via a dedicated `generateDailyReviewProblems` composition function (`src/engine/dailyReview.ts`) — explicit per-grade operation quotas (not generic `operation:'mixed'` random selection), one seeded RNG, one isolated `GenerationHistory`, one deterministic shuffle. That single ordered `Problem[]` is the sole source of truth for interactive practice, same-day replay, print review, and print answer key. `PracticeWidget` gained one small additive `problems?: Problem[]` prop (existing callers unaffected) plus two source-audited runtime fixes that were prerequisites for heterogeneous grades: remainder-input rendering now reads `problem.remainder` instead of session-wide `config.withRemainder`, and `WrittenProblemInput`'s answer cap was raised from 3 to 4 digits (both verified behavior-preserving for every existing page). PB/streak/progress-link UI is suppressed via the existing `sessionPresentation="shared"` path; Daily Review intentionally stays out of `/progress/`/`ALL_PRESETS` in V1, with per-grade completion state stored independently (completing one grade never marks another complete). No secondary shuffle/new-set action, no projector-mode toggle (the existing prototype surface is already classroom-display-capable), untimed only, no accounts/backend. Discoverability: a homepage callout, a For Teachers classroom-workflow step and link, and a grade-prefilled `?grade=` CTA on all five existing grade pages (client-side-only preselection; canonical stays query-string-agnostic). 21 new tests added (`tests/daily-review.test.ts`) covering exact quotas, operand ranges, no-negative-subtraction, exact/remainder division correctness, determinism across date/grade/mix-version, and independent per-grade completion storage; `tests/routes-components.test.ts` and `scripts/test-build-contract.mjs` extended for route/discoverability/canonical/sitemap coverage. Full suite: 211/211 tests passing, clean `npm run build`, `npm run test:build-contract` passing.
